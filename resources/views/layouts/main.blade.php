...
<head>
  ...
  <title>CLSUHub - @yield('title')</title>
  <!-- Bootstrap CSS -->
      ...
      <div class="row align-items-center">
        ...
        <div class="col-md-3 d-flex align-items-center">
          ...
          <a href="{{ url('/forum') }}" class="d-flex align-items-center text-decoration-none">
            <img src="{{ asset('css/uploads/logo1.png') }}" alt="CLSUHub Logo" class="site-logo d-none d-md-block">
            <img src="{{ asset('css/uploads/logo3.png') }}" alt="CLSUHub Logo" class="site-logo d-md-none">
          </a>
        </div>
        
        <!-- Search Bar -->
        <div class="col-md-5">
          <form action="{{ route('forum.index') }}" method="GET" class="w-100">
        ...
        <div class="col-md-4 d-flex justify-content-end align-items-center">
          <div class="dropdown profile-dropdown">
            <div class="d-flex align-items-center dropdown-toggle profile-toggle" id="profileDropdown" data-toggle="dropdown" aria-expanded="false">
              <img src="{{ auth()->user()->profile_photo 
              ? asset('storage/'.auth()->user()->profile_photo) 
              : 'https://ui-avatars.com/api/?name='.urlencode(auth()->user()->name).'&rounded=true' }}" 
              class="profile-avatar" alt="Profile">
  
              <span class="user-name d-none d-sm-inline text-truncate">{{ auth()->user()->name }}</span>
              <i class="fas fa-chevron-down ml-1 dropdown-arrow"></i>
            </div>

            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="profileDropdown">
              @if(auth()->user()->role === 'faculty')
                <a class="dropdown-item dropdown-hide-mobile" href="{{ url('/faculty/dashboard') }}">
                  <i class="fas fa-chart-line"></i> Dashboard
                </a>
                <div class="dropdown-divider dropdown-hide-mobile"></div>
              @endif

              <a class="dropdown-item dropdown-hide-mobile" href="{{ url('/forum') }}">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Close sidenav when clicking outside (overlay)
  sidenavOverlay.addEventListener('click', closeSidenav);

  // Optional: Close sidenav on window resize if screen is wider than 768px
  window.addEventListener('resize', function() {
    if (window.innerWidth > 768 && sidenav.classList.contains('open')) {
      closeSidenav();
    }

    // Also update logo based on screen width
    updateLogoForMobile();
  });

  // Optional: Close sidenav on load if open and on mobile
  if (window.innerWidth <= 768 && sidenav.classList.contains('open')) {
    closeSidenav();
  }
  ...
  function updateLogoForMobile() {
    const regularLogo = document.querySelector('.desktop-logo');
    let mobileLogo = document.querySelector('.mobile-logo');

    if (window.innerWidth <= 768) {
      if (!mobileLogo && regularLogo) {
        const newMobileLogo = document.createElement('img');
        newMobileLogo.src = regularLogo.src.replace('logo1.png', 'logo3.png');
        newMobileLogo.alt = 'CLSUHub Mobile Logo';
        newMobileLogo.className = 'mobile-logo sidenav-logo';
